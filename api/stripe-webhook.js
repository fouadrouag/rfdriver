...
function recapRows(m) {
  const rows = [
    ...
    ['Prix', `${m.prix} — payé par carte ✅`],
    ['Mode de paiement', 'Carte bancaire (en ligne, via Stripe)'],
...
export default async function handler(req, res) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    if (m.business !== 'rfdriver') {
      return res.status(200).json({ received: true, skipped: 'not-rfdriver' });
    }
